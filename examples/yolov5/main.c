...
int main(int argc, char **argv) {
    ...
    while (!g_sigint_received) {
        ...
        if (supervisor.state == SUPERVISOR_PLAY && supervisor.play_mode == PLAY_LOOP_CHASE && smoothed.has_cat) {
            ...
            laser_enabled = true;
        } else if (supervisor.state == SUPERVISOR_PLAY && supervisor.play_mode == PLAY_LOOP_BAIT &&
            ...
            algo_name = "bait";
            laser_enabled = true;
        } else if (supervisor.state == SUPERVISOR_WAKE || supervisor.state == SUPERVISOR_DETECT) {
        ...
        if (supervisor.room_empty_confirmed || supervisor.state == SUPERVISOR_IDLE ||
            supervisor.state == SUPERVISOR_SLEEP || deadman_active) {
            laser_enabled = false;
        }
