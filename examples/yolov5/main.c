...
int main(int argc, char **argv) {
    ...
    struct CatPlayState play_state;
    cv::Point2f laser_estimate(0.0f, 0.0f);
    int frame_index = 0;
    ...
    while (1) {
        ...
        if (smoothed.has_cat) {
            ...
            cv::Point2f play_target = build_cat_play_target(
                &play_state, smoothed, laser_estimate, frame_index,
                frame.cols, frame.rows, control_dt_sec, &algo_name);
            laser_estimate = play_target;
            ...
            fprintf(stderr,
                    "cat_conf=%.2f algo=%s engage=%.2f target=(%.1f,%.1f) servo=(%.2f,%.2f)\n",
                    smoothed.confidence, algo_name, play_state.engagement_score,
