...
int main(int argc, char **argv) {
    ...
    fprintf(stderr,
            "Servo init: PAN(chip=%u,channel=%u) TILT(chip=%u,channel=%u).\n",
            pan_pwm_chip, pan_pwm_channel, tilt_pwm_chip, tilt_pwm_channel);

    if (servo_pwm_open(&pan_pwm, pan_pwm_chip, pan_pwm_channel) < 0) {
        fprintf(stderr, "Servo init failed: PAN open failed. Shutting down.\n");
    ...
    fprintf(stderr, "Servo init status: PAN open ok.\n");

    if (servo_pwm_open(&tilt_pwm, tilt_pwm_chip, tilt_pwm_channel) < 0) {
        fprintf(stderr, "Servo init failed: TILT open failed. Shutting down.\n");
        print_pwm_sysfs_overview();
        mosfet_gpio_close(&pan_power_gpio);
        mosfet_gpio_close(&tilt_power_gpio);
        mosfet_gpio_close(&laser_gpio);
    ...
    fprintf(stderr, "Servo init status: TILT open ok.\n");

    if (servo_pwm_set_angle(&pan_pwm, 0.0f) < 0) {
        fprintf(stderr, "Servo init failed: PAN set-angle failed. Shutting down.\n");
        print_pwm_sysfs_overview();
        mosfet_gpio_close(&pan_power_gpio);
        mosfet_gpio_close(&tilt_power_gpio);
        mosfet_gpio_close(&laser_gpio);
    ...
    fprintf(stderr, "Servo init status: PAN angle set to center.\n");

    if (servo_pwm_set_angle(&tilt_pwm, 0.0f) < 0) {
        fprintf(stderr, "Servo init failed: TILT set-angle failed. Shutting down.\n");
        print_pwm_sysfs_overview();
        mosfet_gpio_close(&pan_power_gpio);
        mosfet_gpio_close(&tilt_power_gpio);
        mosfet_gpio_close(&laser_gpio);
    ...
    fprintf(stderr, "Servo init status: TILT angle set to center.\n");

    if (servo_pwm_enable(&pan_pwm) < 0) {
        fprintf(stderr, "Servo init failed: PAN enable failed. Shutting down.\n");
        print_pwm_sysfs_overview();
        mosfet_gpio_close(&pan_power_gpio);
        mosfet_gpio_close(&tilt_power_gpio);
        mosfet_gpio_close(&laser_gpio);
    ...
    fprintf(stderr, "Servo init status: PAN enable ok.\n");

    if (servo_pwm_enable(&tilt_pwm) < 0) {
        fprintf(stderr, "Servo init failed: TILT enable failed. Shutting down.\n");
        print_pwm_sysfs_overview();
        mosfet_gpio_close(&pan_power_gpio);
        mosfet_gpio_close(&tilt_power_gpio);
        mosfet_gpio_close(&laser_gpio);
    ...
    fprintf(stderr, "Servo init status: TILT enable ok.\n");
