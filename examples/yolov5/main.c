...
static void load_pwm_mapping_from_env(unsigned int *pan_chip,
                                      unsigned int *pan_channel,
                                      unsigned int *tilt_chip,
                                      unsigned int *tilt_channel) {
    const char *pan_chip_env = getenv("YOLOV5_PAN_PWM_CHIP");
    const char *pan_channel_env = getenv("YOLOV5_PAN_PWM_CHANNEL");
    const char *tilt_chip_env = getenv("YOLOV5_TILT_PWM_CHIP");
    const char *tilt_channel_env = getenv("YOLOV5_TILT_PWM_CHANNEL");

    if (pan_chip_env) *pan_chip = (unsigned int)strtoul(pan_chip_env, NULL, 10);
    if (pan_channel_env) *pan_channel = (unsigned int)strtoul(pan_channel_env, NULL, 10);
    if (tilt_chip_env) *tilt_chip = (unsigned int)strtoul(tilt_chip_env, NULL, 10);
    if (tilt_channel_env) *tilt_channel = (unsigned int)strtoul(tilt_channel_env, NULL, 10);
}
...
int main(int argc, char **argv) {
    ...
    unsigned int pan_pwm_chip = 10;
    unsigned int pan_pwm_channel = 0;
    unsigned int tilt_pwm_chip = 10;
    unsigned int tilt_pwm_channel = 1;
    ...
    load_pwm_mapping_from_env(&pan_pwm_chip, &pan_pwm_channel, &tilt_pwm_chip, &tilt_pwm_channel);

    fprintf(stderr,
            "PWM config: PAN(chip=%u,channel=%u) TILT(chip=%u,channel=%u)\n",
    ...
    // Immediate hardware sanity check: sweep both axes once at startup.
    // This helps validate PWM channel mapping even before cat detection starts.
