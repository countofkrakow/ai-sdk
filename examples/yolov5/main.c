...
static void handle_signal_stop(int signo) {
    (void)signo;
    g_stop_requested = 1;
}

static void run_laser_circle_test(struct ServoPwm *pan_pwm,
                                  struct ServoPwm *tilt_pwm,
                                  struct MosfetPowerGpio *laser_gpio) {
    const float pan_radius_deg = 28.0f;
    const float tilt_radius_deg = 20.0f;
    const float angular_speed_rad_per_sec = 1.0f;
    const useconds_t sleep_us = 30000;

    struct sigaction sa;
    memset(&sa, 0, sizeof(sa));
    sa.sa_handler = handle_signal_stop;
    ...
    mosfet_gpio_set(laser_gpio, true);
    fprintf(stderr,
            "--test mode active: MOSFET rails ON, laser ON, circular scan running (Ctrl+C to exit).\n");

    struct timespec ts;
    clock_gettime(CLOCK_MONOTONIC, &ts);
    const double start_sec = (double)ts.tv_sec + (double)ts.tv_nsec / 1e9;

    while (!g_stop_requested) {
        clock_gettime(CLOCK_MONOTONIC, &ts);
        const double now_sec = (double)ts.tv_sec + (double)ts.tv_nsec / 1e9;
        const float angle = (float)((now_sec - start_sec) * angular_speed_rad_per_sec);

        const float pan = pan_radius_deg * cosf(angle);
        const float tilt = tilt_radius_deg * sinf(angle);
...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (strcmp(camera_device, "/dev/video0") == 0 &&
            parse_brightness_percent(argv[i], &laser_brightness_percent) != 0) {
            camera_device = argv[i];
            continue;
        }

        if (parse_brightness_percent(argv[i], &laser_brightness_percent) != 0) {
            fprintf(stderr,
                    "Invalid argument '%s'.\n"
                    "Usage: %s <nbg> [camera_device] [laser_brightness_percent] [--test]\n"
                    "  nbg: path to YOLOv5 .nb model (optional in --test mode)\n"
                    "  camera_device: optional V4L2 node (default: /dev/video0)\n"
                    "  laser_brightness_percent: optional integer 0..100 (default: 100)\n"
                    "  --test: skip camera/NPU and scan laser in a circle to validate MOSFET switching\n",
                    argv[i], argv[0]);
            return -1;
        }
    }

    if (!test_mode && nbg == NULL) {
        fprintf(stderr,
                "Usage: %s <nbg> [camera_device] [laser_brightness_percent] [--test]\n"
