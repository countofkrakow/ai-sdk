...
static void rank_alternative_algorithms_by_engagement(
    const struct CatPlayState *state,
    enum CatPlayAlgorithm *first,
    enum CatPlayAlgorithm *second) {
    // Lower engagement => favor novelty (dart/zigzag). Higher engagement => gentler oval.
    if (state->algorithm == CAT_PLAY_OVAL) {
        if (state->engagement_score < 0.35f) {
            *first = CAT_PLAY_STARE_DART;
            *second = CAT_PLAY_ZIGZAG_RETREAT;
        } else {
            *first = CAT_PLAY_ZIGZAG_RETREAT;
            *second = CAT_PLAY_STARE_DART;
        }
        return;
    }

    if (state->algorithm == CAT_PLAY_STARE_DART) {
        if (state->engagement_score > 0.65f) {
            *first = CAT_PLAY_OVAL;
            *second = CAT_PLAY_ZIGZAG_RETREAT;
        } else {
            *first = CAT_PLAY_ZIGZAG_RETREAT;
            *second = CAT_PLAY_OVAL;
        }
        return;
    }
...
static void maybe_transition_with_engagement(struct CatPlayState *state, int base_percent) {
    enum CatPlayAlgorithm first = CAT_PLAY_OVAL;
    enum CatPlayAlgorithm second = CAT_PLAY_STARE_DART;
    rank_alternative_algorithms_by_engagement(state, &first, &second);

    // With low engagement, increase transition probability to seek a better response.
    const float novelty_boost = (1.0f - state->engagement_score) * 25.0f;
    const int tuned_percent = (int)clampf_local((float)base_percent + novelty_boost, 5.0f, 85.0f);
    if ((rand() % 100) >= tuned_percent) {
        return;
    }

    // Engagement ranking: first choice is favored, but keep some variety.
    state->algorithm = ((rand() % 100) < 70) ? first : second;
}
...
cv::Point2f build_cat_play_target(
    ...
    if (state->cat_still_time_sec >= 60.0f) {
        maybe_transition_with_engagement(state, 40);
        state->cat_still_time_sec = 0.0f;
    }
        ...
        *algo_name_out = "stare_dart";
        if (state->stare_dart_phase == STARE_DART_HOLD) {
            ...
            if (state->stare_dart_hold_time_sec <= 0.0f) {
                ...
                maybe_transition_with_engagement(state, 10);
            }
