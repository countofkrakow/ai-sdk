    ...
    while (1) {
        ...
        if (!frame.empty() && cv::imwrite(args->frame_file, frame)) {
            ...
            if (input_tensor_bytes != NULL) {
                ...
                debug_trace_log(args->trace, DEBUG_LOG_WARN, "INFER", "Pre-process failed for frame %lu", frame_seq);
            }
        } else if (frame.empty()) {
            debug_trace_log(args->trace, DEBUG_LOG_WARN, "INFER", "Skipping empty frame %lu", frame_seq);
        } else {
            debug_trace_log(args->trace, DEBUG_LOG_WARN, "INFER", "Failed to write frame %lu to %s", frame_seq, args->frame_file);
        }
...
static int read_next_frame(AppRuntime *rt, FrameInputs *in) {
    ...
    } else if (rt->cfg.test_mode) {
        in->frame_bgr = cv::Mat::zeros(480, 640, CV_8UC3);
    } else {
        cv::Mat raw_frame;
